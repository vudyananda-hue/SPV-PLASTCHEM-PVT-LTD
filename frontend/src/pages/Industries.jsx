...
function IndustryRow({ industry, i }) {
  ...
  return (
    <div
      ref={ref}
      className={`grid lg:grid-cols-2 gap-12 items-center mb-20 last:mb-0 ${!isEven ? 'lg:direction-rtl' : ''}`}
      id={`industry-${industry.id}`}
    >
      <div className={`scroll-animate scroll-cinematic-up ${isVisible ? 'is-visible' : ''} ${!isEven ? 'lg:order-2' : ''}`}>
      ...
      <div
        className={`scroll-animate scroll-cinematic-zoom ${isVisible ? 'is-visible' : ''} relative rounded-2xl overflow-hidden min-h-[300px] ${!isEven ? 'lg:order-1' : ''}`}
        style={{ background: `linear-gradient(135deg, ${i % 2 === 0 ? '#0A1628, #1B2D4A' : '#111D38, #0D162E'})` }}
      >
    ...
    <>
      ...
      <section className="section-padding bg-white">
        <div className="container-wide mx-auto">
          {industries.map((industry, i) => (
            <IndustryRow key={industry.id} industry={industry} i={i} />
          ))}
        </div>
      </section>
      ...
      <section className="py-16" style={{ background: 'linear-gradient(135deg, #0A1628 0%, #1B2D4A 100%)' }}>
        <div className="container-wide mx-auto px-4 sm:px-6 text-center">
          ...
          <Link to="/contact" className="btn-primary btn-shimmer text-base !py-3 !px-8">
